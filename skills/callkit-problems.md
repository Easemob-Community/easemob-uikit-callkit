...
description: >
  ...
  触发时机：用户遇到类似问题、需要排查已知 bug、评估架构风险。
  关联文档：.agent/refactor-guide.md（重构陷阱）、.agent/current-state.md（已知问题）
---
...
一对一视频通话正常建立后，对方（尤其是 iOS 端）快速关闭再打开摄像头，Web 端大窗（远程视频区域）会突然变成自己的画面。刷新页面后可能恢复，但反复操作后高概率复现。

### 根因

**1. Provider 重复初始化**

`EasemobChatCallKitProvider.vue` 在 `onMounted` 和 `watch(chatClientStore.getChatClient)` 里都会调用 `initCore()`。第一次初始化完成后，chatClient 的变化会触发第二次初始化，导致 `CallKitCore` 被销毁重建，`RtcService` 单例状态也发生重置。

**2. `playRemoteVideo` 并发执行**

iOS 端快速开关摄像头时，`user-published` / `user-unpublished` 事件可能在 100ms 内连续到达。多个 `playRemoteVideo` 并发执行，内部都执行 `currentRemoteVideoTrack.stop()` + `remoteVideoTrack.play()`，Agora 的同一个远程轨道被反复 stop/play，渲染状态错乱。
...
Store 与 `createPinia()` 必须来自同一个 Pinia 实例。由于 Pinia 的 `piniaSymbol` 是模块内通过 `Symbol()` 生成的，只要存在两份 Pinia 代码（callkit 内部 inline 一份、项目又单独装了一份），它们的 symbol 就不匹配。
...
发版时如果没有人工去改这两个常量，就会出现"包已经升到 2.0.6，日志还是 2.0.0"的误导现象。

### 修复方案（2.0.6 已实施）

在 `vite.lib.config.ts` 中通过 Vite `define` 从 `package.json` 注入版本号：
...
- 信令协议层面是否需要在服务端做 resourceId 路由优化
- 评估是否把 `playRemoteVideo` 这类带副作用的 RTC 操作抽象为队列或状态机
- 评估是否需要服务端支持 "设备级离线消息过滤"
- 评估是否把 Pinia 从 inline bundle 改回 peerDependency

---

## 相关文档

- **AI 重构规范**：`.agent/refactor-guide.md`
