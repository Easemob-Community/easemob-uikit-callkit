...
description: >
  ...
  触发时机：用户询问如何接入 CallKit、遇到接入错误、需要配置帮助。
  关联文档：.agent/AGENTS.md（项目结构速查）
---
...
| **`conversationId`** | `string` | **会话 ID**：单聊=对方用户ID，群聊=groupId |
| **`isLocal`** | `boolean` | **方向标识**：`true`=本端行为触发 |
| **`localUserRole`** | `string` | **当前用户角色**：`'caller'` / `'callee'` / `'participant'` |
...
| `agoraClient` | `IAgoraRTCClient` | ❌ | Agora 客户端实例。推荐传入 |
| `initConfig` | `object` | ❌ | 配置项 |
| `getUserInfo` | `function` | ❌ | 自定义用户资料查询 |
...
| `enableIDBLog` | `boolean` | `true` | IndexedDB 日志持久化 |
